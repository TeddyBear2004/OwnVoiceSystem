...
public class OvCommand implements CommandExecutable {
    private final static EmbedCreateSpec OV_COMMANDS;
    private final static EmbedCreateSpec OV_COMMANDS_FOR_ADMINS;
        ...
        return ((Member) user).getBasePermissions()
                .flatMap(permissions ->
                        permissions.contains(Permission.MANAGE_GUILD)
                                ? messageChannel.createMessage(OV_COMMANDS_FOR_ADMINS)
                                : messageChannel.createMessage(OV_COMMANDS));
    }
}
